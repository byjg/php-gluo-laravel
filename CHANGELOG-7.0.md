...
## Continuous Integration caveat

PHP 8.6 is **excluded from the build matrix**, though the composer constraint allows it.
`orchestra/testbench` pulls `laravel/framework` → `league/commonmark` → `league/config` →
`nette/schema`, and `nette/schema` caps at `php 8.1 - 8.5`. The runtime dependencies
(`illuminate/*`) install on 8.6 without complaint — verified in `byjg/php:8.6-cli` — so
consumers on 8.6 are unaffected; only the test harness cannot run there yet.
